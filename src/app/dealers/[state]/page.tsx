      ...
      <section className="py-12 sm:py-16 border-b border-ink-100">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <h1 className="font-serif text-display-sm text-ink-900">
            Gun Shops, Ranges &amp; Instructors in {state.name}
          </h1>
          <div className="w-10 h-0.5 bg-ink-900 mt-6 mb-4" />
          <p className="text-sm text-ink-400">
            {stateDealers.length} {stateDealers.length === 1 ? "listing" : "listings"} in {stateCities.length} {stateCities.length === 1 ? "city" : "cities"}
          </p>
        </div>
      </section>
